...
/*  若有订单改价后，仅显示在右上角支付金额栏，仅显示!!!! */
$query_changeprice  = "select totalprice from weixin_commonshop_changeprices where status=true and isvalid=true and batchcode='" . $post_data['batchcode_arr'][0]['batchcode'] . "'";
$changeprice_result = _mysql_query($query_changeprice) or die('Query failed:' . mysql_error());

while ($row = mysql_fetch_assoc($changeprice_result)) {
    $change_price = $row['totalprice'];
}

if (!empty($change_price)) {
    $show_price = $change_price;
}
...
<body>
    <div class="list-one popup-menu-title" id="needpay">
        ...
        <p><span class="suppspanl">订单需付金额</span><span class="suppspanr"><?php echo $show_price; ?><small class="suppspansma"><?php echo OOF_T ?></small></span></p>
    </div>
                ...
                if(result.errcode==0) {
                    if (result.datalist.length == 0) {
                        ...
                        for (i = 0; i < result.datalist.length; i++) {
                            ...
                            }else if($pay_type=="moneybag" && $balance<=0){
                                content += '<div class = "new_popup-menu-row" data-value="' + $pay_type + '">';
                            }else if($pay_type=="card" && $balance<=0){
                                content += '<div class = "new_popup-menu-row" data-value="' + $pay_type + '">';
                            }else{
                                if($pay_type=="virtual"){
                            ...
                            if($pay_type=="IPSpay" && price<2){
                                content += '<div class="newdiv"><p class="newfont"  style="color: #bbb;">' + result.datalist[i].pay_name + ($show_balance == true  ? " (余额：<?php if (OOF_P != 2) {
                                    echo OOF_S;
                                }?>"+$balance+"<?php if (OOF_P == 2) {
                                    echo OOF_S;
                                }?>)" : "") +
                                '</p>';
                            }else if($pay_type=="moneybag" && $balance<=0){
                                ...
                                content += '<div class="newdiv"><p class="newfont">' + result.datalist[i].pay_name + ($show_balance == true  ? " (余额：<?php if (OOF_P != 2) {
                                    echo OOF_S;
                                }?>"+$balance+"<?php if (OOF_P == 2) {
                                    echo OOF_S;
                                }?>)" : "") +
...
function new_popup(obj,paytype_obj,extra_obj){
    ...
    $.ajax({
        ...
        success:function(result){
            ...
            $.ajax({
                ...
                type: 'post',
                async : false,
                data:{'customer_id_en':customer_id_en,'industry_type':industry_type,"order_batchcode":order_batchcode,"internal_opt":"get_returnpage","return_batchcode":result.batchcode,"city_type":city_type,"cash_o2o_type":cash_o2o_type,"orderingretail_opt":opt,"return_status":result.status,"supplier_id":supplier_id,"user_id":pay_user_id},
...
var pay_type_obj = {};
var extra_info_obj = {};
function toPayPage(pay_type,paytype_obj,extra_obj){
                        ...
                        if(pay_type =='moneybag') {
                            ...
                            function (res) {
                                ...
                                if (res.isopen_poundage == 1) {
                                    ...
                                    $("#gyuji").show();
                                    var html = '<p style="text-align:center;font-wight:normal;">订单金额：<?php if (OOF_P != 2) {
                                        echo OOF_S;
                                    }?>&nbsp;' + res.pay_price + '&nbsp;<?php if (OOF_P == 2) {
                                        echo OOF_S;
                                    }?></p>';
                                    html += '<p style="text-align:center;">手续费：<?php if (OOF_P != 2) {
                                        echo OOF_S;
                                    }?>&nbsp;' + res.poundage + '&nbsp;<?php if (OOF_P == 2) {
                                        echo OOF_S;
                                    }?></p>';
                                    html += '<p style="text-align:center;color:#333333">合计：<font color=#FF8530><?php if (OOF_P != 2) {
                                        echo OOF_S;
                                    }?>&nbsp;' + all_pay + '&nbsp;<?php if (OOF_P == 2) {
                                        echo OOF_S;
                                    }?></font></p>';
     ...
     * */
     function StartPay(paytype_obj,extra_obj){
        ...
        if(paytype_obj.pay_type == "moneybag" || paytype_obj.pay_type == "card" || paytype_obj.pay_type == "virtual" || paytype_obj.pay_type == "nopay" || paytype_obj.pay_type == "account"){
            ...
            var url="";
            if( paytype_obj.pay_type =='blueoceanpay' ){//蓝海支付
            ...
            }else if( paytype_obj.pay_type =='weipay' ){//微信支付
                //var pay_code = get_paycode(pay_user_id,industry_type);
            ...
            $.ajax({
                ...
                dataType: 'json',
                type: 'post',
                data:{'customer_id_en':customer_id_en,'industry_type':industry_type,"order_batchcode":order_batchcode,"internal_opt":"get_returnpage","city_type":city_type,"cash_o2o_type":cash_o2o_type,"orderingretail_opt":opt,"user_id":pay_user_id},
                success:function(data){
